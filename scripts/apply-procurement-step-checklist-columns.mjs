/**
 * รัน migration checklist columns บน procurement_steps
 * ใช้: SUPABASE_DB_PASSWORD=your-db-password node scripts/apply-procurement-step-checklist-columns.mjs
 */
...
if (!password) {
  console.error(
    "ต้องระบุรหัสผ่าน Database: SUPABASE_DB_PASSWORD=... node scripts/apply-procurement-step-checklist-columns.mjs",
  );
  process.exit(1);
}
...
const sql = fs.readFileSync(
  "scripts/sql/add-procurement-step-checklist-columns.sql",
  "utf8",
);
...
for (const connectionString of connectionCandidates) {
  ...
  try {
    ...
    const verify = await client.query(
      `select column_name, data_type
       ...
       where table_schema = 'public'
         and table_name = 'procurement_steps'
         and column_name in (
           'step1_checklist',
           'step2_checklist',
           'step3_checklist',
    ...
    console.log("OK: procurement_steps checklist columns migration applied");
