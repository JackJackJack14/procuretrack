...
const CONSTRUCTION_ACTIVITY_PATTERN = /ก่อสร้าง|งานก่อสร้าง|construction/i;

/**
 * ตรวจจับประเภทงานอัตโนมัติจากข้อมูลโครงการหลัก — ไม่ให้ผู้ใช้เลือกซ้ำในขั้นตอนที่ 10
 */
export function resolveProjectWorkType(project: ProjectWorkTypeSource): Step10ProjectType {
  const activity = project.activity_type?.trim() ?? "";
  if (activity && CONSTRUCTION_ACTIVITY_PATTERN.test(activity)) {
    return "construction";
  }
  if (project.design_code?.trim()) {
    return "construction";
  }
  if (project.method === "e_bidding") {
    return "construction";
  }
  return "general";
}
...
export function isProjectConstructionWork(project: ProjectWorkTypeSource): boolean {
  return resolveProjectWorkType(project) === "construction";
}
